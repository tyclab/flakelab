...
There is no host-side provisioning script here — rebuild this box with `flakelab update`. The WSL-only verbs (`provision`, `build-distro`, `test-provision`, `distro-name`) are not on `flakelab --help`; nothing here needs them. A reboot ends every session on the box, so it is never an agent step, same as on any other machine.
