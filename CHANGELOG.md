...
- Activation no longer restarts the backup units or the ssh-agent (`X-SwitchMethod=keep-old`). Restarting a oneshot that is mid-run BLOCKS the switch until it finishes, so any state-sync run longer than home-manager's 5-minute start timeout failed the whole switch — and each retry cancelled the running sync and started a fresh one, re-arming the same timeout. Restarting the ssh-agent silently emptied its loaded keys mid-session on every package bump. In all three units the reloaded definition applies on the next timer fire (agent: the next user-manager start), which is when it would first matter anyway. Because keep-old also means nothing external ever clears a wedged run, both backup services now carry an explicit `TimeoutStartSec` (2h full, 30min state-only) — a oneshot left `activating` blocks its own timer forever.
- The transcript push leg skips sources that cannot produce a new write: a scan cache (`~/.local/state/flakelab/state-sync/scan-cache.tsv`, size+mtime per synced transcript) records what the gate last passed, so a run re-stages and re-scans only transcripts that changed. Before, every run copied and gitleaks-scanned EVERY local transcript, a cost that grew with session history until a short sync interval spent most of each period re-scanning settled bytes. A verdict depends on the rulings as well as the bytes, so the cache carries an epoch hash of the gitleaks config and both decisions files and drops every entry when any of them changes — a ruling from `--review-secrets` always reaches the next run. A transcript held back whole is never cached and re-scans every run. One visible change: an unchanged, already-synced transcript with a redaction no longer re-warns in every run's journal — the pending review stays visible in `held.jsonl` and `flakelab doctor`.
