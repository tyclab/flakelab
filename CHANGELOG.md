...
- `flakelab sessions`: the Claude Code sessions running in this distro, with the session id each process holds open in `/proc` — the newest transcript in a directory is not it, two sessions in one directory share that. `--save` writes `<dir>  <id>` lines (to the state root when one is configured, else `~/.claude/sessions/`) so a `wsl --shutdown` or reboot costs nothing but the restart; `--resume` prints the `cd … && claude --resume <id>` lines back. Born on 2026-09-02, when a stack reset on a tower needed a reboot and four shells had no known ids.
