...
`enhancement` · phases 1 to 3 done, the rest designed
...
`auto` with its timer and the quota hook, the suite. Open: the store in
`flakelab backup`, `run`/`env` profiles, the Codex and Kiro adapters, the
statusline.
