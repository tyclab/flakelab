...
`enhancement` · phases 1 to 4 done, the rest designed
...
`auto` with its timer and the quota hook, `run`/`env` profiles, the suite.
Open: the store in `flakelab backup`, the Codex and Kiro adapters, the
statusline.
