...
function New-OverlaySkeleton([string]$root, [string]$flakeText) {
    ...
    # behind, no drift check). An existing .git is the operator's and gets no
    ...
    Do-Step "git init $root (one commit, no remote)" {
        ...
        if ($LASTEXITCODE -ne 0) { Warn "git commit failed in $root - the repository is initialised, nothing is committed" }
    }
}
